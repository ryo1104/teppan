class StripeExtAccountForm
  ...
  def create_stripe_bank_inputs
    bank = Bank.find_by(name: self.bank_name)
    branch = bank.branches.find_by(name: self.branch_name)
    if bank.present? && branch.present?
      stripe_bank_info = {
        external_account:
        {
          ...
          account_holder_name: self.account_holder_name, # カタカナでなければStripeエラー
          currency: 'jpy',
          country: 'jp'
        },
      ...
      [false, "cannot retrieve bank #{self.bank_name} or branch #{self.branch_name}"]
    end
  end
